...
//
// Demonstrates the basics to get DirectX 11 Compute Shader (aka DirectCompute)
// up and running by implementing Array A + Array B
//
...
// If defined, then the hardware/driver must report support for double-precision
// CS 5.0 shaders or the sample fails to run
//#define TEST_DOUBLE
...
//--------------------------------------------------------------------------------------
HRESULT CreateComputeDevice(_Outptr_ ID3D11Device **ppDeviceOut,
                            _Outptr_ ID3D11DeviceContext **ppContextOut,
                            _In_ bool bForceRef);
HRESULT CreateComputeShader(_In_z_ LPCWSTR pSrcFile,
                            _In_z_ LPCSTR pFunctionName,
                            _In_ ID3D11Device *pDevice,
                            _Outptr_ ID3D11ComputeShader **ppShaderOut);
HRESULT CreateStructuredBuffer(_In_ ID3D11Device *pDevice,
                               _In_ UINT uElementSize, _In_ UINT uCount,
                               _In_reads_(uElementSize *uCount) void *pInitData,
                               _Outptr_ ID3D11Buffer **ppBufOut);
HRESULT CreateRawBuffer(_In_ ID3D11Device *pDevice, _In_ UINT uSize,
                        _In_reads_(uSize) void *pInitData,
                        _Outptr_ ID3D11Buffer **ppBufOut);
HRESULT CreateBufferSRV(_In_ ID3D11Device *pDevice, _In_ ID3D11Buffer *pBuffer,
                        _Outptr_ ID3D11ShaderResourceView **ppSRVOut);
HRESULT CreateBufferUAV(_In_ ID3D11Device *pDevice, _In_ ID3D11Buffer *pBuffer,
                        _Outptr_ ID3D11UnorderedAccessView **pUAVOut);
ID3D11Buffer *
CreateAndCopyToDebugBuf(_In_ ID3D11Device *pDevice,
...
void RunComputeShader(_In_ ID3D11DeviceContext *pd3dImmediateContext,
                      ...
                      _In_ UINT nNumViews,
                      _In_reads_(nNumViews)
                          ID3D11ShaderResourceView **pShaderResourceViews,
                      _In_opt_ ID3D11Buffer *pCBCS,
                      _In_reads_opt_(dwNumDataBytes) void *pCSData,
                      _In_ DWORD dwNumDataBytes,
                      _In_ ID3D11UnorderedAccessView *pUnorderedAccessView,
                      _In_ UINT X, _In_ UINT Y, _In_ UINT Z);
HRESULT FindDXSDKShaderFileCch(_Out_writes_(cchDest) WCHAR *strDestPath,
                               _In_ int cchDest, _In_z_ LPCWSTR strFilename);
...
ID3D11Query *pQueryDisjoint2 = nullptr;
ID3D11Query *pQueryTimestampStart = nullptr;
ID3D11Query *pQueryTimestampEnd = nullptr;
ID3D11Query *pQueryTimestampStart2 = nullptr;
ID3D11Query *pQueryTimestampEnd2 = nullptr;
...
int g_count = 0;
void runCPU(int count) {
  double sum = 0;
  auto start = std::chrono::steady_clock::now();
  for (long i = 1; i <= 10000000 * count; ++i) {
    sum += sqrt(i);
  }
...
int __cdecl main() {
  ...
  if (FAILED(CreateComputeShader(L"SLM_8X8_4X16.hlsl", "CSMain", g_pDevice,
                                 &g_pCS)))
    return 1;
  ...
  CreateStructuredBuffer(g_pDevice, ComponentSize * sizeof(float),
                         NUM_ELEMENTS / ComponentSize, &g_vBuf0[0], &g_pBuf0);
  CreateStructuredBuffer(g_pDevice, ComponentSize * sizeof(float),
                         NUM_ELEMENTS / ComponentSize, &g_vBuf1[0], &g_pBuf1);
  CreateStructuredBuffer(g_pDevice, ComponentSize * sizeof(float),
                         NUM_ELEMENTS / ComponentSize, nullptr, &g_pBufResult);
...
#else
  CreateRawBuffer(g_pDevice, NUM_ELEMENTS * sizeof(float), &g_vBuf0[0],
                  &g_pBuf0);
  CreateRawBuffer(g_pDevice, NUM_ELEMENTS * sizeof(float), &g_vBuf1[0],
                  &g_pBuf1);
  ...
  if (g_pBuf0)
    g_pBuf0->SetPrivateData(WKPDID_D3DDebugObjectName, sizeof("Buffer0") - 1,
                            "Buffer0");
  if (g_pBuf1)
    g_pBuf1->SetPrivateData(WKPDID_D3DDebugObjectName, sizeof("Buffer1") - 1,
                            "Buffer1");
  if (g_pBufResult)
    g_pBufResult->SetPrivateData(WKPDID_D3DDebugObjectName,
                                 sizeof("Result") - 1, "Result");
#endif
...
#if defined(_DEBUG) || defined(PROFILE)
  if (g_pBuf0SRV)
    g_pBuf0SRV->SetPrivateData(WKPDID_D3DDebugObjectName,
                               sizeof("Buffer0 SRV") - 1, "Buffer0 SRV");
  if (g_pBuf1SRV)
    g_pBuf1SRV->SetPrivateData(WKPDID_D3DDebugObjectName,
                               sizeof("Buffer1 SRV") - 1, "Buffer1 SRV");
  if (g_pBufResultUAV)
    g_pBufResultUAV->SetPrivateData(WKPDID_D3DDebugObjectName,
                                    sizeof("Result UAV") - 1, "Result UAV");
#endif
  ...
  g_pDevice->CreateQuery(&queryDesc, &pQueryDisjoint2);
  ...
  for (int it = 0; it < computeCount; it++) {
    ...
    RunComputeShader(g_pContext, g_pCS, 2, aRViews, nullptr, nullptr, 0,
                     g_pBufResultUAV, OutputN / TSN, OutputM / TSM, 1);
...
//--------------------------------------------------------------------------------------
// Create the D3D device and device context suitable for running Compute
// Shaders(CS)
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT
CreateComputeDevice(ID3D11Device **ppDeviceOut,
                    ID3D11DeviceContext **ppContextOut, bool bForceRef) {
  *ppDeviceOut = nullptr;
  *ppContextOut = nullptr;
  ...
  if (!bForceRef) {
    ...
    if (SUCCEEDED(hr)) {
      // A hardware accelerated device has been created, so check for Compute
      // Shader support
      ...
      if (flOut < D3D_FEATURE_LEVEL_11_0) {
#ifdef TEST_DOUBLE
        bNeedRefDevice = true;
        printf("No hardware Compute Shader 5.0 capable device found (required "
               "for doubles), trying to create ref device.\n");
#else
        // Otherwise, we need further check whether this device support CS4.x
        // (Compute on 10)
        ...
        if (!hwopts
          ...
          bNeedRefDevice = true;
          printf("No hardware Compute Shader capable device found, trying to "
                 "create ref device.\n");
        }
#endif
      }
        ...
        (*ppDeviceOut)
            ->CheckFeatureSupport(D3D11_FEATURE_DOUBLES, &hwopts,
                                  sizeof(hwopts));
        if (!hwopts.DoublePrecisionFloatShaderOps) {
          bNeedRefDevice = true;
          printf("No hardware double-precision capable device found, trying to "
                 "create ref device.\n");
        }
      }
#endif
  ...
  if (bForceRef || FAILED(hr) || bNeedRefDevice) {
    // Either because of failure on creating a hardware device or hardware
    // lacking CS capability, we create a ref device here
    ...
    hr = D3D11CreateDevice(
        nullptr,                   // Use default graphics card
        D3D_DRIVER_TYPE_REFERENCE, // Try to create a hardware accelerated
                                   // device
        nullptr,        // Do not use external software rasterizer module
        uCreationFlags, // Device creation flags
        flvl, sizeof(flvl) / sizeof(D3D_FEATURE_LEVEL),
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT
CreateComputeShader(LPCWSTR pSrcFile, LPCSTR pFunctionName,
                    ID3D11Device *pDevice, ID3D11ComputeShader **ppShaderOut) {
  if (!pDevice || !ppShaderOut)
    return E_INVALIDARG;

  // Finds the correct path for the shader file.
  // This is only required for this sample to be run correctly from within the
  // Sample Browser, in your own projects, these lines could be removed safely
  WCHAR str[MAX_PATH];
  ...
  // We generally prefer to use the higher CS shader profile when possible as
  // CS 5.0 is better performance on 11-class hardware
  LPCSTR pProfile = (pDevice->GetFeatureLevel() >= D3D_FEATURE_LEVEL_11_0)
                        ? "cs_5_0"
                        : "cs_4_0";

  ID3DBlob *pErrorBlob = nullptr;
  ID3DBlob *pBlob = nullptr;
  hr = D3DCompileFromFile(str, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE,
                          pFunctionName, pProfile, dwShaderFlags, 0, &pBlob,
                          &pErrorBlob);
  if (FAILED(hr)) {
    if (pErrorBlob)
  ...
  hr = pDevice->CreateComputeShader(
      pBlob->GetBufferPointer(), pBlob->GetBufferSize(), nullptr, ppShaderOut);
  ...
  if (SUCCEEDED(hr)) {
    (*ppShaderOut)
        ->SetPrivateData(WKPDID_D3DDebugObjectName, lstrlenA(pFunctionName),
                         pFunctionName);
  }
#endif
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT CreateStructuredBuffer(ID3D11Device *pDevice,
                                                      UINT uElementSize,
                                                      UINT uCount,
                                                      void *pInitData,
...
// Create Raw Buffer
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT CreateRawBuffer(ID3D11Device *pDevice,
                                               UINT uSize, void *pInitData,
                                               ID3D11Buffer **ppBufOut) {
  *ppBufOut = nullptr;
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT
CreateBufferSRV(ID3D11Device *pDevice, ID3D11Buffer *pBuffer,
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT
CreateBufferUAV(ID3D11Device *pDevice, ID3D11Buffer *pBuffer,
  ...
  if (descBuf.MiscFlags & D3D11_RESOURCE_MISC_BUFFER_ALLOW_RAW_VIEWS) {
    ...
    desc.Format =
        DXGI_FORMAT_R32_TYPELESS; // Format must be DXGI_FORMAT_R32_TYPELESS,
    ...
    desc.Format =
        DXGI_FORMAT_UNKNOWN; // Format must be must be DXGI_FORMAT_UNKNOWN, when
                             // creating a View of a Structured Buffer
...
//--------------------------------------------------------------------------------------
// Create a CPU accessible buffer and download the content of a GPU buffer into
// it This function is very useful for debugging CS programs
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ ID3D11Buffer *
CreateAndCopyToDebugBuf(ID3D11Device *pDevice,
...
#if defined(_DEBUG) || defined(PROFILE)
    debugbuf->SetPrivateData(WKPDID_D3DDebugObjectName, sizeof("Debug") - 1,
                             "Debug");
#endif
...
RunComputeShader(ID3D11DeviceContext *pd3dImmediateContext,
                 ID3D11ComputeShader *pComputeShader, UINT nNumViews,
                 ID3D11ShaderResourceView **pShaderResourceViews,
                 ID3D11Buffer *pCBCS, void *pCSData, DWORD dwNumDataBytes,
                 ID3D11UnorderedAccessView *pUnorderedAccessView, UINT X,
                 UINT Y, UINT Z) {
  pd3dImmediateContext->CSSetShader(pComputeShader, nullptr, 0);
  pd3dImmediateContext->CSSetShaderResources(0, nNumViews,
                                             pShaderResourceViews);
  pd3dImmediateContext->CSSetUnorderedAccessViews(0, 1, &pUnorderedAccessView,
                                                  nullptr);
  pd3dImmediateContext->CSSetConstantBuffers(0, 1, &g_pConstantBuffer);
  if (pCBCS && pCSData) {
    D3D11_MAPPED_SUBRESOURCE MappedResource;
    pd3dImmediateContext->Map(pCBCS, 0, D3D11_MAP_WRITE_DISCARD, 0,
                              &MappedResource);
  ...
  pd3dImmediateContext->CSSetUnorderedAccessViews(0, 1, ppUAViewnullptr,
                                                  nullptr);
...
_Use_decl_annotations_ HRESULT FindDXSDKShaderFileCch(WCHAR *strDestPath,
  ...
  swprintf_s(strDestPath, cchDest, L"%s\\..\\..\\%s\\%s", strExePath,
             strExeName, strFilename);
  if (GetFileAttributes(strDestPath) != 0xFFFFFFFF)
    return S_OK;
